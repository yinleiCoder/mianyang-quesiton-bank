-- 0061: 首页改成"答题热力图"——practice_dashboard 给本月的**逐日答题数**。
--
-- 用户要的是 GitHub 那种按天染色、答得越多颜色越深的热力图（不是日历格子）。
-- 原先 0059 加的 checkin_days 只有"哪几天练过"、没有次数，画不出深浅，
-- 所以换成 month_daily：[{date, count}]，只含本月有作答的日子（前端按 0 补空）。
-- checkin_days / checkin_month 一并撤掉：它们只服务那个已被替换的日历组件。
--
-- 窗口就是**本月**（用户定的），不做 12 周/一年——首页那张卡放不下，也不值得。
--
-- 本函数最新版在 0059，下面整体覆盖，只动签到那一段与返回字段。
...
begin
  ...
  -- 本月逐日答题数（热力图用）。与 daily 的区别：daily 是**近 14 天**（趋势图要连续补零），
  -- 这里是**本月**、且只给有作答的日子（没作答的前端按 0 处理，省得每月传 31 条零）。
  -- 热力图要的是"哪天答了多少"，所以这里必须带 count，不能只给日期。
