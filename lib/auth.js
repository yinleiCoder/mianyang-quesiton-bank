...
export const getAuthContext = cache(async () => {
  ...
  return {
    // 下游只用到 .id / .email / .phone，不再透传整个 auth user 对象。
    // email 优先取 JWT claim（签发时的事实），取不到再退到 profiles.email —— 两个来源
    // 都在手边，兜一下免得某个签发路径不带 email 时页面上静默变成空白。
    //
    // 注意：手机号账号的 email **是合成地址**（`138…@phone.myquiz.cn`，见 lib/phone.js），
    // 展示前必须过 displayEmail / displayIdentifier 折叠掉。phone 只有 profiles 这一个来源
    // （合成邮箱方案下 auth.users.phone 恒为 NULL）。
    user: {
      id: claims.sub,
