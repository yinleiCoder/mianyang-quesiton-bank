...
// 可建试卷节点：**任意层级**——题只挂末端的课程/公共学科，卷子可以挂在专业大类/专业层，
// 卷内含其下各课程的题（服务端 check_can_author_paper 同口径；科目筛选与组卷库都按子树命中）
const PAPER_NODE_KINDS = ["discipline", "category", "major", "course"]
export function isPaperNode(kind) {
  return PAPER_NODE_KINDS.includes(kind)
}
