...
// 「一键入库」的范围：**市级专家环节的内容任务**——通过即发布入库、全市可见。
// 组长环节通过只是把题转给专家（不等于入库）；上下线/恢复事件是题目级事件，也不在此列。
// 这个判据同时被审批收件箱（从 mineRows 里筛）与「我的题目」（单独查一次）使用，
// 放这里是为了两处的口径不漂。
const isPublishable = (a) => a.kind === "content" && a.stage === "city"

// 收件箱行 → 可批量入库的任务 id（行是 assemble() 出来的 { approval, … }）
