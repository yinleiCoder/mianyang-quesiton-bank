...
export async function loadMyQuestions(supabase, uid) {
  ...
  const [vRes, nRes, sRes, aRes] = await Promise.all([
    ...
    supabase
      .from("approvals")
      .select("id, question_id, kind, stage, assigned_user_ids")
      .eq("state", "waiting")
  ...
  for (const a of aRes.data ?? []) {
    if (a.kind === "content") {
      ...
      assigneeIds.push(...(a.assigned_user_ids ?? []))
    ...
    return {
      ...
      pendingApproval: pendingContent.has(q.id)
        ? {
            ...
            assignedName: (pendingContent.get(q.id).assigned_user_ids ?? [])
