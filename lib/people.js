...
export async function loadPeople(supabase, uids, schools = null) {
  ...
  const [pResRaw, rRes, aRes] = await Promise.all([
    supabase
      .from("profiles")
      .select("user_id, name, email, phone, school_id, avatar_url, is_admin, identity")
      .in("user_id", ids),
  ...
  const pRes = pResRaw.error
    ? await supabase
        .from("profiles")
        .select("user_id, name, phone, school_id, avatar_url, is_admin, identity")
        .in("user_id", ids)
    : pResRaw
  ...
  for (const p of profiles) {
    ...
    out.set(p.user_id, {
      ...
      name: p.name ?? "",
      email: p.email ?? "",
      // phone 与 email 是**两个独立字段**。手机号账号的 profiles.email 存的是合成邮箱
      // （见 lib/phone.js），展示层要用 displayEmail/displayIdentifier 折叠掉，
      // 别直接把 email 印出来。
      phone: p.phone ?? "",
