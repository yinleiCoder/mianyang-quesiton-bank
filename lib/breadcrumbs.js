...
const RULES = [
  ...
  // 子路径必须排在 /papers/[^/]+ 之前：RULES.find 取首个命中
  { re: /^\/papers\/[^/]+\/board$/, crumbs: () => [C("组卷库", "/papers"), C("成绩排行")] },
  ...
  // 班级学情看板。父级写「学生」而不是「管理台」：用它的主力是教师（建班才是管理员的事）
  { re: /^\/classes\/[^/]+$/, crumbs: () => [C("学生", "/students"), C("班级学情")] },

  // 讲评模式（/lecture/paper/[id]）**刻意不登记**：它是全屏投屏页、不在 (app) 组里，
  // 那套布局与面包屑都不会渲染它。登记进来只会让人以为它挂在侧栏导航下。
